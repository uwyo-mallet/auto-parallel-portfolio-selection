...
def SubmitJob(SatSolvers,Partition,Node,TestInstances,Path,Argument,InstanceName):
    ...
    solver_path = "/gscratch/hkashgar/instances/SATCompetition2016Application/compiled_solvers/"
    ...
    for i in range(len(TestInstances)):
        ...
        for k in range(len(SatSolvers)):
            ...
            solver = SatSolvers[k]
            print("solver:" + solver)
            TestString += " /usr/bin/time -o {}__{}.log {} {} | tail -n 200 > {}__{}.output ".format(solver, Instance, solver_path + "/" + SatSolvers[k] + "/" + solver_post_path, TestInstances[i], solver, Instance)
