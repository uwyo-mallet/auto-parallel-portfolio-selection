set_paths = function(){
  ...
  if(Sys.info()['sysname'][[1]]=="Linux"){
    if(file.exists("~/Documents/OrganizedScripts/OverheadResults.R")){
      ...
      modelPath = "/gscratch/hkashgar/mlr-scripts/SAT11-INDU/Prediction/StandardError/"
    }
    ...
    modelPath = "C:/Users/hnyk9/Thesis/mlr-scripts/Prediction/SAT11-INDU/StandardError/"
  }
}
...
check_codes = function(){
  # checking the code
  # ---------------
  self = PredictionResults$new(benchmarks_name = "SAT11-INDU")
  ...
  self$par10_CSV
  self$get_mcp_dataframe(sequentialData = SequentialPerformance$new(benchmarks_name = "SAT11-INDU"))
  self$mcp_CSV
...
training = function(){
  ...
  self = SequentialPerformance$new(benchmarks_name = "SAT11-INDU")
  ...
  self = PredictionResults$new("SAT11-INDU")
...
get_top_vbs = function(saved = TRUE){
  ...
  if(!saved){
    ...
    for(i in range){
      par = ParallelLevel$new(benchmarks_name = "SAT11-INDU", cores = i)
      top_n_vbs = vector()
      for(j in 1:i){
    ...
    write.csv(top_vbs, "~/Documents/OrganizedScripts/results/SAT11-INDU/top_vbs.csv", row.names = FALSE)
  } else{
    top_vbs = read.csv("~/Documents/OrganizedScripts/results/SAT11-INDU/top_vbs.csv")
  }
  return(top_vbs)
}
...
get_top_sbs = function(saved = TRUE){
  if(!saved){
    ...
    for(i in range){
      ...
      top_n_sbs
      par = ParallelLevel$new(benchmarks_name = "SAT11-INDU",i)
    ...
    write.csv(top_sbs, "~/Documents/OrganizedScripts/results/SAT11-INDU/top_sbs.csv", row.names = FALSE)
  } else{
    top_sbs = read.csv("~/Documents/OrganizedScripts/results/SAT11-INDU/top_sbs.csv")
  }
  return(top_sbs)
}
...
get_top_AS_noUncertainty_noSplitting = function(saved = TRUE){
  ...
  if(!saved){
    for(i in range){
      selectionPath = paste("/home/haniye//Documents/OrganizedScripts/results/SAT11-INDU/selections_algorithmSelection_noUncertainty/",i,"_cores/",sep="")
    ...
    write.arff(summary,"~/Documents/OrganizedScripts/results/SAT11-INDU/top_as_noUncertainty.arff")
  } else{
    for(i in range){
      selectionPath = paste("/home/haniye//Documents/OrganizedScripts/results/SAT11-INDU/selections_algorithmSelection_noUncertainty/",i,"_cores/",sep="")
      summary = self$get_summary_selection_result(ignoreTimeouts = FALSE,
...
get_time_splitting_prediction = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT11-INDU/time_splitting/",c,"_cores/",sep="")
...
get_time_splitting_prediction_SE = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT11-INDU/time_splitting/pred+SE/",c,"_cores/",sep="")
      self$time_splitting_scheduling(predictionPath = self$predictionPath, 
                                     selectionPath = path_to_schedule, 
                                     cores = c,
                                     ignoreTimeoutsOnVBS = FALSE, 
...
get_flexfolio_3s_results = function(){
  ...
  for(c in range){
    ...
    csv$claspfolio[which(csv$claspfolio == 50000)] <- 5000
...
get_joint_probability_optimum = function(){
  ...
  range = range[1:101]
  for(r in range){
    dir.create(paste("~/Documents/OrganizedScripts/results/SAT11-INDU/JointProbability/tau_",r,sep = ""))
    self$selection_based_on_SE(predictionPath = self$predictionPath,
                               saveTo = paste("~/Documents/OrganizedScripts/results/SAT11-INDU/JointProbability/tau_",r,sep=""),
                               method_number = 5,
                               top_selection = 10,
                               ignoreTimeoutsOnVBS = FALSE,
                               orderBy = "pred",
                               ...
                               alpha = 0,
                               JP_limit = r)
    summary = self$get_summary_selection_result( paste("~/Documents/OrganizedScripts/results/SAT11-INDU/JointProbability/tau_",r,sep=""),
                                                 ignoreTimeouts = FALSE,
                                                 method_number = 5,
                                                 median = FALSE)
    print(r)
...
get_joint_probability_results = function(saved = TRUE, optimum = 0.63){
  self = PredictionResults$new("SAT11-INDU")
  range = c(1:10)
  resR = data.frame(matrix(ncol= 11,nrow=0))
  resMCP = data.frame(matrix(ncol= 11,nrow=0))
  ...
  if(!saved){
    for(i in range){
      self$selection_based_on_SE(predictionPath = self$predictionPath,
                                 saveTo = paste("~/Documents/OrganizedScripts/results/SAT11-INDU/joint_optimal/",i, "-core",sep=""),
                                 method_number = 5,
                                 top_selection = i,
                                 ignoreTimeoutsOnVBS = FALSE,
                                 orderBy = "pred",
                                 ...
                                 alpha = 0,
                                 JP_limit = optimum)
      summary = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT11-INDU/joint_optimal/",i,"-core",sep=""),
                                                  ignoreTimeouts = FALSE,
                                                  method_number = 5,
                                                  median = FALSE)
      summary_med = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT11-INDU/joint_optimal/",i,"-core",sep=""),
                                                      ignoreTimeouts = FALSE,
                                                      method_number = 5,
                                                      median = TRUE)
      resR = rbind(resR, summary[[1]])
...
get_joint_probability_results_averageOpt = function(saved = TRUE, optimum = 0.82){
  ...
  if(!saved){
    for(i in range){
      self$selection_based_on_SE(predictionPath = self$predictionPath,
                                 saveTo = paste("~/Documents/OrganizedScripts/results/SAT11-INDU/joint_optimal_avg/",i, "-core",sep=""),
                                 method_number = 5,
                                 top_selection = i,
                                 ignoreTimeoutsOnVBS = FALSE,
                                 orderBy = "pred",
                                 ...
                                 alpha = 0,
                                 JP_limit = optimum)
      summary = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT11-INDU/joint_optimal_avg/",i,"-core",sep=""),
                                                  ignoreTimeouts = FALSE,
                                                  method_number = 5,
                                                  median = FALSE)
      summary_med = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT11-INDU/joint_optimal_avg/",i,"-core",sep=""),
                                                      ignoreTimeouts = FALSE,
                                                      method_number = 5,
                                                      median = TRUE)
      resR = rbind(resR, summary[[1]])
...
get_optimum_tau = function(){
  ...
  # theta = read.csv("~/Documents/OrganizedScripts/results/SAT11-INDU//RandomSearch_SAT11-INDU_combined_allEqual_NOTignoreTO_300.csv")
  ...
  # range =c(1:10)
  # theta = 0.0625
  # selectionPath = "~/Documents/OrganizedScripts/results/SAT11-INDU/selections_optimal_theta_0.0625/selection_optimal_theta_7_cores/"
  # self = PredictionResults$new("SAT11-INDU")
  ...
  # 
  # write.arff(summary,"~/Documents/OrganizedScripts/results/SAT11-INDU/top_Uncertainty_Theta_0.1098642_limitingSolvers.arff")
...
plot_all_results = function(save = FALSE, optimum = 0.852, metric = "Runtime"){
  ...
  if(metric == "Runtime"){
    p <- ggplot(results, aes(x=cores, y=Runtime, colour = Approach, shape = Approach)) +
      ...
      labs(x = "Cores", y = "Runtime (s)", title = "SAT11-INDU-MAIN")+theme(plot.title = element_text(hjust = 0.5))+
    ...
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT11-INDU_line_chart_parallel_runtime.pdf")
    }
  } else if(metric == "MCP"){
    p <- ggplot(results, aes(x=cores, y=MCP, colour = Approach, shape = Approach)) +
      ...
      guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5),
             colour = guide_legend(keywidth = 3, keyheight = 1.5))+
      expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
      labs(x = "Cores", y = "MCP", title = "SAT11-INDU-MAIN")+theme(plot.title = element_text(hjust = 0.5))+
      scale_x_continuous(breaks=c(1:10))+
      scale_color_discrete(labels=c(bquote(AS[1]), 'Single Best Solver',
                                    "Timesplitting_preds", "Timesplitting_preds+SE",
                                    bquote(AS[~p[~'\u2229']]), 'Virtual Best Solver'))+
      ...
      scale_y_continuous(trans='log10')
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT11-INDU_line_chart_parallel_MCP.pdf")
    }
    ...
    p <- ggplot(results, aes(x=cores, y=PAR10, colour = Approach, shape = Approach)) +
      ...
      guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5),
             colour = guide_legend(keywidth = 3, keyheight = 1.5))+
      expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
      labs(x = "Cores", y = "PAR10", title = "SAT11-INDU-MAIN")+theme(plot.title = element_text(hjust = 0.5))+
      scale_x_continuous(breaks=c(1:10))+
      scale_color_discrete(labels=c(bquote(AS[1]), 'Single Best Solver',
                                    "Timesplitting_preds", "Timesplitting_preds+SE",
                                    bquote(AS[~p[~'\u2229']]), 'Virtual Best Solver'))+
      ...
      scale_y_continuous(trans='log10')
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT11-INDU_line_chart_parallel_PAR10.pdf")
    }
  }
  return(p)
}

plot_all_results_normalized_gap = function(save = FALSE, optimum = 0.852){
  #vbs is 1
  #sbs is 0
  #sbs-value/sbs-vbs
  results = scrape_all_results()
  ...
  par10_sbs[which(par10_sbs==5000)] <- 50000
  par10_sbs = mean(par10_sbs)
  par10_vbs = get_top_vbs()$cores_1
  par10_vbs[which(par10_vbs==5000)] <- 50000
  ...
  p = ggplot(results, aes(x=cores, y=PAR10, colour = Approach, shape = Approach)) +
    ...
    guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5), 
           colour = guide_legend(keywidth = 3, keyheight = 1.5))+
    expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
    labs(x = "Processors", y = "Normalized Gap Closed", title = "SAT11-INDU")+
    ...
    scale_shape_discrete(labels=c("3S",bquote(AS[1]), 'SBS',"Time Splitting",
                                  bquote(AS[~p[~'\u2229']]), 'VBS'))
  if(save){
    ggsave(dpi = 500, width = 9, height = 5, filename = "SAT11-INDU_line_chart_parallel_NormalizedGap.pdf")  
  }
  return(p)
}
...
results = scrape_all_results()
results$scenario = "SAT11-INDU"

setwd("~/Documents/OrganizedScripts/results/")
write.csv(results,'summary_results_all_SAT11-INDU.csv',row.names = FALSE)
