set_paths = function(){
  ...
  if(Sys.info()['sysname']=="Linux"){
    if(file.exists("~/Documents/auto-parallel-portfolio-selection/Scripts/OverheadResults.R")){
      source("~/Documents/auto-parallel-portfolio-selection/OverheadResults.R")
    ...
    predictionPath = "C:/Users/hnyk9/Thesis/OrganizedScripts/SAT2018/preds/"
    selectionPath = "C:/Users/hnyk9/Thesis/OrganizedScripts/SAT2018/selection/"
    finalSelectionPath = "C:/Users/hnyk9/Thesis/OrganizedScripts/SAT2018/final/"
    modelPath = "C:/Users/hnyk9/Thesis/mlr-scripts/Prediction/SAT2018/StandardError/"
  }
}
...
get_top_vbs = function(saved = TRUE){
  ...
  if(!saved){
    ...
    top_vbs = read.csv("~/Documents/OrganizedScripts/results/SAT2018/top_vbs.csv")
  }
  return(top_vbs)
}
...
get_top_sbs = function(saved = TRUE){
  if(!saved){
    ...
    top_sbs = read.csv("~/Documents/OrganizedScripts/results/SAT2018/top_sbs.csv")
  }
  return(top_sbs)
}
...
get_top_AS_noUncertainty_noSplitting = function(saved = TRUE){
  ...
  if(!saved){
    ...
    write.arff(summary,"~/Documents/OrganizedScripts/results/SAT2018/top_as_noUncertainty.arff")
  } else{
    for(i in range){
      selectionPath = paste("/home/haniye//Documents/OrganizedScripts/results/SAT2018/selections_algorithmSelection_noUncertainty/",i,"_cores/",sep="")
      summary = self$get_summary_selection_result(ignoreTimeouts = FALSE,
...
get_time_splitting_prediction = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT2018/time_splitting/",c,"_cores/",sep="")
...
get_time_splitting_prediction_SE = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT2018/time_splitting/pred+SE/",c,"_cores/",sep="")
      self$time_splitting_scheduling(predictionPath = self$predictionPath, 
                                     selectionPath = path_to_schedule, 
                                     cores = c,
                                     ignoreTimeoutsOnVBS = FALSE, 
...
get_time_splitting_prediction_aSE = function(saved = TRUE, a = 2){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT2018/time_splitting/pred+2SE/",c,"_cores/",sep="")
      self$time_splitting_scheduling(predictionPath = self$predictionPath, 
                                     selectionPath = path_to_schedule, 
                                     cores = c,
                                     ignoreTimeoutsOnVBS = FALSE, 
...
get_time_splitting_cutoff = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT2018/time_splitting/cutoff/",c,"_cores/",sep="")
      self$time_splitting_scheduling(predictionPath = self$predictionPath, 
                                     selectionPath = path_to_schedule, 
                                     cores = c,
                                     ignoreTimeoutsOnVBS = FALSE, 
...
get_flexfolio_3s_results = function(){
  ...
  for(c in range){
    path_to_results = paste("/home/haniye/Documents/auto-parallel-portfolio-selection/Baselines/flexfolio/SAT2018/sat18_results/SAT2018-",
                            c,"core-3s.csv",sep="")
...
get_joint_probability_results = function(saved = TRUE, optimum = 0.81){
  self = PredictionResults$new("SAT2018")
  range = c(1:10)
  resR = data.frame(matrix(ncol= 11,nrow=0))
  resMCP = data.frame(matrix(ncol= 11,nrow=0))
  ...
  if(!saved){
    for(i in range){
      self$selection_based_on_SE(predictionPath = self$predictionPath,
                                 saveTo = paste("~/Documents/OrganizedScripts/results/SAT2018/joint_optimal/",i, "-core",sep=""),
                                 method_number = 5,
                                 top_selection = i,
                                 ignoreTimeoutsOnVBS = FALSE,
                                 orderBy = "pred",
                                 ...
                                 alpha = 0,
                                 JP_limit = optimum)
      summary = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2018/joint_optimal/",i,"-core",sep=""),
                                                  ignoreTimeouts = FALSE,
                                                  method_number = 5,
                                                  median = FALSE)
      summary_med = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2018/joint_optimal/",i,"-core",sep=""),
                                                      ignoreTimeouts = FALSE,
                                                      method_number = 5,
                                                      median = TRUE)
      resR = rbind(resR, summary[[1]])
...
get_joint_probability_results_averageOpt = function(saved = TRUE, optimum = 0.82){
  ...
  if(!saved){
    for(i in range){
      self$selection_based_on_SE(predictionPath = self$predictionPath,
                                 saveTo = paste("~/Documents/OrganizedScripts/results/SAT2018/joint_optimal_avg/",i, "-core",sep=""),
                                 method_number = 5,
                                 top_selection = i,
                                 ignoreTimeoutsOnVBS = FALSE,
                                 orderBy = "pred",
                                 ...
                                 alpha = 0,
                                 JP_limit = optimum)
      summary = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2018/joint_optimal_avg/",i,"-core",sep=""),
                                                  ignoreTimeouts = FALSE,
                                                  method_number = 5,
                                                  median = FALSE)
      summary_med = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2018/joint_optimal_avg/",i,"-core",sep=""),
                                                      ignoreTimeouts = FALSE,
                                                      method_number = 5,
                                                      median = TRUE)
      resR = rbind(resR, summary[[1]])
...
get_optimum_tau = function(){
  ...
  # theta = read.csv("~/Documents/OrganizedScripts/results/SAT2018//RandomSearch_SAT2018_combined_allEqual_NOTignoreTO_300.csv")
  ...
  # range =c(1:10)
  # theta = 0.0625
  # selectionPath = "~/Documents/OrganizedScripts/results/SAT2018/selections_optimal_theta_0.0625/selection_optimal_theta_7_cores/"
  # self = PredictionResults$new("SAT2018")
  ...
  # 
  # write.arff(summary,"~/Documents/OrganizedScripts/results/SAT2018/top_Uncertainty_Theta_0.1098642_limitingSolvers.arff")
...
plot_all_results = function(save = FALSE, optimum = 0.81, metric = "Runtime"){
  ...
  if(metric == "Runtime"){
    p <- ggplot(results, aes(x=cores, y=Runtime, colour = Approach, shape = Approach)) +
      ...
      labs(x = "Cores", y = "Runtime (s)", title = "SAT2018-EXP")+theme(plot.title = element_text(hjust = 0.5))+
    ...
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2018_line_chart_parallel_runtime.pdf")
    }
  } else if(metric == "MCP"){
    p <- ggplot(results, aes(x=cores, y=MCP, colour = Approach, shape = Approach)) +
      ...
      guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5),
             colour = guide_legend(keywidth = 3, keyheight = 1.5))+
      expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
      labs(x = "Cores", y = "MCP", title = "SAT2018-EXP")+theme(plot.title = element_text(hjust = 0.5))+
      scale_x_continuous(breaks=c(1:10))+
      scale_color_discrete(labels=c(bquote(AS[1]), 'Single Best Solver',
                                    "Timesplitting_preds", "Timesplitting_preds+SE",
                                    bquote(AS[~p[~'\u2229']]), 'Virtual Best Solver'))+
      ...
      scale_y_continuous(trans='log10')
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2018_line_chart_parallel_MCP.pdf")
    }
    ...
    p <- ggplot(results, aes(x=cores, y=PAR10, colour = Approach, shape = Approach)) +
      ...
      guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5),
             colour = guide_legend(keywidth = 3, keyheight = 1.5))+
      expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
      labs(x = "Cores", y = "PAR10", title = "SAT2018-EXP")+theme(plot.title = element_text(hjust = 0.5))+
      scale_x_continuous(breaks=c(1:10))+
      scale_color_discrete(labels=c(bquote(AS[1]), 'Single Best Solver',
                                    "Timesplitting_preds", "Timesplitting_preds+SE",
                                    bquote(AS[~p[~'\u2229']]), 'Virtual Best Solver'))+
      ...
      scale_y_continuous(trans='log10')
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2018_line_chart_parallel_PAR10.pdf")
    }
  }
  return(p)
}

plot_all_results_normalized_gap = function(save = FALSE, optimum = 0.81){
  #vbs is 1
  #sbs is 0
  #sbs-value/sbs-vbs
  results = scrape_all_results()
  ...
  p = ggplot(results, aes(x=cores, y=PAR10, colour = Approach, shape = Approach)) +
    ...
    guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5), 
           colour = guide_legend(keywidth = 3, keyheight = 1.5))+
    expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
    labs(x = "Processors", y = "Normalized Gap Closed", title = "SAT2018-EXP")+
    ...
    scale_shape_discrete(labels=c("3S", bquote(AS[1]), 'SBS',
                                  "Time Splitting", bquote(AS[~p[~'\u2229']]), 'VBS'))
  if(save){
    ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2018_line_chart_parallel_NormalizedGap.pdf")  
  }
  return(p)
}
  ...
  for(c in 1:nrow(JoinProbabilityResults[[1]])){
    if(c == 1){
      row = c("Uncertainty - JointProbability", 1, JoinProbabilityResults[[1]][1,]$Parallel_time, JoinProbabilityResults[[2]][1,]$Parallel_time, JoinProbabilityResults[[3]][1,]$Parallel_time, 
              JoinProbabilityResults[[4]][1,]$Parallel_time, JoinProbabilityResults[[5]][1,]$Parallel_time, JoinProbabilityResults[[6]][1,]$Parallel_time,"Uncertainty - JointProbability - optimum = 0.813")
      df_results= rbind(df_results,row)
    } else{ 
      row = c("Uncertainty - JointProbability", c, JoinProbabilityResults[[1]][c,]$Parallel_time, JoinProbabilityResults[[2]][c,]$Parallel_time, JoinProbabilityResults[[3]][c,]$Parallel_time, 
              JoinProbabilityResults[[4]][c,]$Parallel_time, JoinProbabilityResults[[5]][c,]$Parallel_time, JoinProbabilityResults[[6]][c,]$Parallel_time, "Uncertainty - JointProbability - optimum = 0.813")
      df_results= rbind(df_results,row)
    }
  }
...
# a = self$actual_error()
results = scrape_all_results()
results$scenario = "SAT18-EXP"

setwd("~/Documents/OrganizedScripts/results/")

write.csv(results,'summary_results_all_SAT2018.csv',row.names = FALSE)
...
sm =get_time_splitting_prediction_SE(FALSE)
