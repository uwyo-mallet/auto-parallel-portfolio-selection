set_paths = function(){
  ...
  if(Sys.info()['sysname']=="Linux"){
    if(file.exists("~/Documents/auto-parallel-portfolio-selection/Scripts/OverheadResults.R")){
      ...
      predictionPath = "/gscratch/hkashgar/OrganizedScripts/SAT2016/preds/"
      selectionPath = "/gscratch/hkashgar/OrganizedScripts/SAT2016/selection/"
      finalSelectionPath = "/gscratch/hkashgar/OrganizedScripts/SAT2016/final/"
      modelPath = "/gscratch/hkashgar/mlr-scripts/SAT2016/Prediction/StandardError/"
    }
    
  } else{
    source("C:/Users/hnyk9/Thesis/auto-parallel-portfolio-selection/Scripts/OverheadResults.R")
    predictionPath = "C:/Users/hnyk9/Thesis/OrganizedScripts/SAT2016/preds/"
    selectionPath = "C:/Users/hnyk9/Thesis/OrganizedScripts/SAT2016/selection/"
    finalSelectionPath = "C:/Users/hnyk9/Thesis/OrganizedScripts/SAT2016/final/"
    modelPath = "C:/Users/hnyk9/Thesis/mlr-scripts/Prediction/SAT2016/StandardError/"
  }
}
...
get_top_vbs = function(saved = TRUE){
  ...
  if(!saved){
    ...
    top_vbs = read.csv("~/Documents/OrganizedScripts/results/SAT2016/top_vbs.csv")
  }
  return(top_vbs)
}
...
get_top_sbs = function(saved = TRUE){
  if(!saved){
    ...
    top_sbs = read.csv("~/Documents/OrganizedScripts/results/SAT2016/top_sbs.csv")
  }
  return(top_sbs)
}
...
get_top_AS_noUncertainty_noSplitting = function(saved = TRUE){
  ...
  if(!saved){
    for(i in range){
      selectionPath = paste("/home/haniye//Documents/OrganizedScripts/results/SAT2016/selections_algorithmSelection_noUncertainty/",i,"_cores/",sep="")
...
get_time_splitting_prediction = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT2016/time_splitting/",c,"_cores/",sep="")
...
get_time_splitting_prediction_SE = function(saved = TRUE){
  ...
  if(!saved){
    for(c in range){
      path_to_schedule = paste("/home/haniye/Documents/OrganizedScripts/results/SAT2016/time_splitting/pred+SE/",c,"_cores/",sep="")
      self$time_splitting_scheduling(predictionPath = self$predictionPath, 
                                     selectionPath = path_to_schedule, 
                                     cores = c,
                                     ignoreTimeoutsOnVBS = FALSE, 
...
get_flexfolio_3s_results = function(){
  ...
  for(c in range){
    path_to_results = paste("/home/haniye/Documents/auto-parallel-portfolio-selection/Baselines/flexfolio/SAT2016/sat16_results/SAT2016-",
                            c,"core-3s.csv",sep="")
...
get_joint_probability_optimum = function(){
  ...
  for(r in range){
    ...
    self$selection_based_on_SE(predictionPath = self$predictionPath,
                               saveTo = paste("~/Documents/OrganizedScripts/results/SAT2016/JointProbability/tau_",r,sep=""),
                               method_number = 5,
                               top_selection = 10,
                               ignoreTimeoutsOnVBS = FALSE,
                               orderBy = "pred",
                               ...
                               alpha = 0,
                               JP_limit = r)
    summary = self$get_summary_selection_result( paste("~/Documents/OrganizedScripts/results/SAT2016/JointProbability/tau_",r,sep=""),
                                                 ignoreTimeouts = FALSE,
                                                 method_number = 5,
                                                 median = FALSE)
    print(r)
...
get_joint_probability_results = function(saved = TRUE, optimum = 0.33){
  self = PredictionResults$new("SAT2016")
  range = c(1:10)
  resR = data.frame(matrix(ncol= 11,nrow=0))
  resMCP = data.frame(matrix(ncol= 11,nrow=0))
  ...
  if(!saved){
    for(i in range){
      self$selection_based_on_SE(predictionPath = self$predictionPath,
                                 ...
                                 alpha = 0,
                                 JP_limit = optimum)
      summary = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2016/joint_optimal/",i,"-core",sep=""),
                                                  ignoreTimeouts = FALSE,
                                                  method_number = 5,
                                                  median = FALSE)
      summary_med = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2016/joint_optimal/",i,"-core",sep=""),
                                                      ignoreTimeouts = FALSE,
                                                      method_number = 5,
                                                      median = TRUE)
      resR = rbind(resR, summary[[1]])
...
get_joint_probability_results_averageOpt = function(saved = TRUE, optimum = 0.82){
  ...
  if(!saved){
    for(i in range){
      self$selection_based_on_SE(predictionPath = self$predictionPath,
                                 saveTo = paste("~/Documents/OrganizedScripts/results/SAT2016/joint_optimal_avg/",i, "-core",sep=""),
                                 method_number = 5,
                                 top_selection = i,
                                 ignoreTimeoutsOnVBS = FALSE,
                                 orderBy = "pred",
                                 ...
                                 alpha = 0,
                                 JP_limit = optimum)
      summary = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2016/joint_optimal_avg/",i,"-core",sep=""),
                                                  ignoreTimeouts = FALSE,
                                                  method_number = 5,
                                                  median = FALSE)
      summary_med = self$get_summary_selection_result(paste("~/Documents/OrganizedScripts/results/SAT2016/joint_optimal_avg/",i,"-core",sep=""),
                                                      ignoreTimeouts = FALSE,
                                                      method_number = 5,
                                                      median = TRUE)
      resR = rbind(resR, summary[[1]])
...
get_optimum_tau = function(){
  ...
  # range =c(1:10)
  # theta = 0.0625
  # selectionPath = "~/Documents/OrganizedScripts/results/SAT2016/selections_optimal_theta_0.0625/selection_optimal_theta_7_cores/"
  # self = PredictionResults$new("SAT2016")
  ...
  # 
  # write.arff(summary,"~/Documents/OrganizedScripts/results/SAT2016/top_Uncertainty_Theta_0.1098642_limitingSolvers.arff")
...
plot_all_results = function(save = FALSE, optimum = 0.33, metric = "Runtime"){
  ...
  if(metric == "Runtime"){
    p <- ggplot(results, aes(x=cores, y=Runtime, colour = Approach, shape = Approach)) +
      ...
      labs(x = "Cores", y = "Runtime (s)", title = "SAT2016-MAIN")+theme(plot.title = element_text(hjust = 0.5))+
    ...
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2016_line_chart_parallel_runtime.pdf")
    }
  } else if(metric == "MCP"){
    p <- ggplot(results, aes(x=cores, y=MCP, colour = Approach, shape = Approach)) +
      ...
      guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5),
             colour = guide_legend(keywidth = 3, keyheight = 1.5))+
      expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
      labs(x = "Cores", y = "MCP", title = "SAT2016-MAIN")+theme(plot.title = element_text(hjust = 0.5))+
      scale_x_continuous(breaks=c(1:10))+
      scale_color_discrete(labels=c(bquote(AS[1]), 'Single Best Solver',
                                    "Timesplitting_preds", "Timesplitting_preds+SE",
                                    bquote(AS[~p[~'\u2229']]), 'Virtual Best Solver'))+
      ...
      scale_y_continuous(trans='log10')
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2016_line_chart_parallel_MCP.pdf")
    }
    ...
    p <- ggplot(results, aes(x=cores, y=PAR10, colour = Approach, shape = Approach)) +
      ...
      guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5),
             colour = guide_legend(keywidth = 3, keyheight = 1.5))+
      expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
      labs(x = "Cores", y = "PAR10", title = "SAT2016-MAIN")+theme(plot.title = element_text(hjust = 0.5))+
      scale_x_continuous(breaks=c(1:10))+
      scale_color_discrete(labels=c(bquote(AS[1]), 'Single Best Solver',
                                    "Timesplitting_preds", "Timesplitting_preds+SE",
                                    bquote(AS[~p[~'\u2229']]), 'Virtual Best Solver'))+
      ...
      scale_y_continuous(trans='log10')
    if(save){
      ggsave(dpi = 500, width = 9, height = 5, filename = "SAT2016_line_chart_parallel_PAR10.pdf")
    }
  }
  return(p)
}

plot_all_results_normalized_gap = function(save = FALSE, optimum = 0.33){
  #vbs is 1
  #sbs is 0
  #sbs-value/sbs-vbs
  results = scrape_all_results()
  ...
  p = ggplot(results, aes(x=cores, y=PAR10, colour = Approach, shape = Approach)) +
    ...
    guides(linetype=guide_legend(title="",keywidth = 3, keyheight = 1.5), 
           colour = guide_legend(keywidth = 3, keyheight = 1.5))+
    expand_limits(x = 1, y = 0)+ theme(text=element_text(size=22,  family="Times"))+
    labs(x = "Processors", y = "Normalized Gap Closed", title = "SAT2016-MAIN")+
...
results = scrape_all_results()
results$scenario = "SAT16-MAIN"
setwd("~/Documents/OrganizedScripts/results/")

write.csv(results,'summary_results_all_SAT2016.csv',row.names = FALSE)
