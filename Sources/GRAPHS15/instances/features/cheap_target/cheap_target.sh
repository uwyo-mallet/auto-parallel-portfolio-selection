...
#SBATCH --job-name=cheap_target_features
#SBATCH --output=./cheap_target_features.out
#SBATCH --error=./cheap_target_features.error
...
cd "/gscratch/hkashgar/BatchScripts/GRAPHS2015/instances/features/cheap_target"
...
do
	...
	echo "$target.f"
        ts=$(date +%s%N)
	/usr/bin/time -o "$instance.t" $ExecFile $target > "$instance.f"
