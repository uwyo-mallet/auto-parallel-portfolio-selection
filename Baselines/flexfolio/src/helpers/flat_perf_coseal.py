...
def write_perf_isac(instance_dict, solvers):
...
def write_perf_snnap(instance_dict, solvers):
    solvers = list(solvers)
    fp = open("perf.data", "w")
    fp.write("\"\",%s\n" %(",".join(solvers)))
    for inst, perfs in instance_dict.items():
...
def rewrite_feature_values(feature_file, active_features, mode="SNNAP"):
...
def write_cv_csv(cv_file):
    ...
    convert to csv: instance, fold
    
    (we assume that there is only one repetition)
    '''        
    ...
    fp_csv = open("cv.data", "w")
    fp_csv.write("instance,fold\n")
    for data in arff_dict["data"]:
        inst = data[0]
        cv = data[2]
        fp_csv.write("%s,%d\n"%(inst, int(cv))) 
...
cv_file = os.path.join(sys.argv[1], "cv.arff")
...
if OUTPUT == "ISAC":
    write_perf_isac(inst_dict, solvers)
elif OUTPUT == "SNNAP":
    write_perf_snnap(inst_dict, solvers)
    write_cv_csv(cv_file)
